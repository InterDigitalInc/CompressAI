...
class TestExport:
    def _run_export(self, *args, timeout=16):
        cmd = [sys.executable, '-m', 'compressai.utils.export']
        cmd += list(args)
        p = Popen(cmd,
    ...
    def test_missing_filepath(self):
        rc, _, _ = self._run_export()
        assert rc == 2

    def test_invalid_filepath(self, tmpdir):
        rc, _, _ = self._run_export(tmpdir)
        assert rc == 1

        p = tmpdir.join('hello.txt')
        p.write('')
        rc, _, _ = self._run_export(p)
        assert rc == 1
    ...
    def test_valid(self, tmpdir):
        ...
        rc, _, _ = self._run_export(p, '--architecture', 'factorized-prior',
                                    '--dir', tmpdir)
        assert rc == 0
    ...
    def test_invalid_model(self, tmpdir):
        ...
        rc, _, _ = self._run_export(p, '--architecture', 'foobar')
        assert rc == 2
    ...
    def test_load(self, tmpdir):
        ...
        for k in ['network', 'state_dict']:
            ...
            rc, _, _ = self._run_export(p, '--architecture',
